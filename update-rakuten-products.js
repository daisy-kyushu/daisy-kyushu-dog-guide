...
function scoreItem(item) {
  ...
  let score = reviewAverage * 20 + Math.min(reviewCount, 2000) / 10;
  if (price > 0) score += 5;
  return score;
}

async function searchRakuten(q) {
  ...
  const items = (data.Items || []).map(e => e.Item);
  if (!items.length) return null;
  return items.sort((a, b) => scoreItem(b) - scoreItem(a))[0];
}

async function main() {
  if (!APP_ID || !AFF_ID) {
    console.log("RAKUTEN_APP_ID / RAKUTEN_AFFILIATE_ID が未設定です。GitHub Secretsを確認してください。");
    return;
  }
  ...
  for (const q of queries) {
    try {
      const item = await searchRakuten(q);
      await sleep(1300); // QPSを低く保つ
      if (!item) continue;
      ...
      updated.push({
        ...
        target: q.keyword,
        dogSize: q.keyword.includes("大型犬") ? ["大型犬"] : ["小型犬", "中型犬", "大型犬"],
        scene: q.scene,
        normalUrl: item.itemUrl,
        rakutenAffiliateUrl: item.affiliateUrl || item.itemUrl,
        rating: item.reviewAverage || "",
        reviewCount: item.reviewCount || "",
        itemPrice: item.itemPrice || "",
        shopName: item.shopName || "",
        mediumImageUrls: (item.mediumImageUrls || []).map(x => x.imageUrl),
        whyCandidate: "楽天APIでレビュー件数・評価を参考に自動選定した候補です。購入前にリンク先で最新情報を確認してください。",
        lastChecked: today,
        status: "楽天API取得済み",
        memo: "価格・在庫・評価は変動します。断定表現を避けて紹介してください。"
      });
    } catch (err) {
      console.error(`Failed: ${q.keyword}`, err.message);
    }
  }

  if (updated.length) {
    await fs.writeFile(DATA_PATH, JSON.stringify(updated, null, 2), "utf8");
    console.log(`products.json を ${updated.length} 件で更新しました。`);
  } else {
    console.log("更新対象の商品がありませんでした。");
  }
}
